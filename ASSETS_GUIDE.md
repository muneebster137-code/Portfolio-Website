...
3. **Interactive Reels**: Loaded from `/images/[slug]-reel.mp4` (only supported for specific video-first brands).
...
- `aba-group-reel.mp4` (Custom daily video strategy campaign reel)
...
| Project / Client Name | Project Slug | Header Image | Artifact Image Filenames | Interactive Video Reel (MP4) |
| :--- | :--- | :--- | :--- | :--- |
...
| **Trenddeck** | `trenddeck` | `trenddeck.webp` | `trenddeck-artifact-1.webp`<br>`trenddeck-artifact-2.webp`<br>`trenddeck-artifact-3.webp`<br>`trenddeck-artifact-4.webp` | **`trenddeck-reel.mp4`** *(9:16 vertical)* |
| **ABA Group** | `aba-group` | `aba-group.webp` | `aba-group-artifact-1.webp`<br>`aba-group-artifact-2.webp`<br>`aba-group-artifact-3.webp` | **`aba-group-reel.mp4`** *(9:16 vertical)* |
| **Treats for Life** | `treats-for-life` | `treats-for-life.webp` | `treats-for-life-artifact-1.webp`<br>`treats-for-life-artifact-2.webp`<br>`treats-for-life-artifact-3.webp`<br>`treats-for-life-artifact-4.webp` | **`treats-for-life-reel.mp4`** *(9:16 vertical)* |
...
| **Anebos Studios** | `anebos-studios` | `anebos-studios.webp` | `anebos-studios-artifact-1.webp`<br>`anebos-studios-artifact-2.webp`<br>`anebos-studios-artifact-3.webp`<br>`anebos-studios-artifact-4.webp` | **`anebos-studios-reel.mp4`** *(9:16 vertical)* |
...
3. **Video Format**: Use standard **`.mp4`** format with H.264 video compression for vertical reels to ensure cross-device mobile compatibility. Keep file sizes small (ideally < 10MB) for quick buffering on mobile networks.
4. **Fallback Mechanics**: 
    ...
    - If a video reel `.mp4` is missing, the phone mockup automatically hides the video player and falls back to a styled soundwave placeholder with premium micro-animations.
